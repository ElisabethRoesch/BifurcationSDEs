...
function plot_slice_comp(alphas, x, ind_pot, ind_quasi)
    ...
    p1 = plot(x[inds], y[inds], label = "", grid = "off", ylim = (-10, 150))
    ...
    p2 = plot(ar[inds2], y2[inds2], label = "", grid = "off")
    return p1, p2
end
...
ps = plot(neg_pot_p, neg_quasi_p, zero_pot_p, zero_quasi_p, pos_pot_p, pos_quasi_p, layout = (3, 2), size = (1000, 1000))
