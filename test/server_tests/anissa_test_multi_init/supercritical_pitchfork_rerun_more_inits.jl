...
file_path_raw = string("plots/data/rerun_longer_time_span/rep_output_", bifur_type, "_raw_rerun_more_inits.jld")
file_path_potential = string("plots/data/rerun_longer_time_span/rep_output_", bifur_type, "_potential_rerun_more_inits.jld")
...
ps_potential = plot_potentials(rep_output_potential, test_list_alpha, test_list_sigma, 1, length(test_list_alpha), 1, 4, cols, alphas_col)
    ...
    savefig(string("plots/", bifur_type, "_summary_plot_2_potential_rerun_more_sdes.pdf"))
...
ps_raw = plot_raw_data(rep_output_raw, test_list_alpha, test_list_sigma, 1, length(test_list_alpha), 1, 4, cols, alphas_col)
    ...
    savefig(string("plots/", bifur_type, "_summary_plot_2_raw_rerun_more_sdes.pdf"))

KS_res = KS_vectors(rep_output_raw)
p = plot_KS_vectors(KS_res, test_list_sigma, test_list_alpha)
    p_all =plot(p[1], p[2], layout = (1, 2))
    savefig(string("plots/", bifur_type, "_KS_raw_data_rerun_more_sdes.pdf"))
KS_res = KS_vectors(rep_output_potential)
p = plot_KS_vectors(KS_res, test_list_sigma, test_list_alpha)
    p_all = plot(p[1], p[2], layout = (1, 2))
    savefig(string("plots/", bifur_type, "_KS_potential_data_rerun_more_sdes.pdf"))

H_res = H_vectors(rep_output_raw)
p = plot_H_vectors(H_res, test_list_sigma, test_list_alpha)
    p_all = plot(p[1], p[2], layout = (1, 2))
    savefig(string("plots/", bifur_type, "_hell_raw_data_rerun_more_sdes.pdf"))

H_res = H_vectors(rep_output_potential)
p = plot_H_vectors(H_res, test_list_sigma, test_list_alpha)
    p_all = plot(p[1], p[2], layout = (1, 2))
    savefig(string("plots/", bifur_type, "_hell_potential_data_rerun_more_sdes.pdf"))
...
p = plot_E_vectors(E_res, test_list_sigma, test_list_alpha)
    p_all = plot(p[1], p[2], layout = (1, 2))
    savefig(string("plots/", bifur_type, "_entropy_single_raw_data_rerun_more_sdes.pdf"))


P_res = pairwise_ent(rep_output_raw)
p = plot_P_vectors(P_res, test_list_sigma, test_list_alpha)
    p_all = plot(p[1], p[2], layout = (1, 2))
    savefig(string("plots/", bifur_type, "_entropy_pairwise_raw_data_rerun_more_sdes.pdf"))


ps_test = plot_summary_stat(E_res, test_list_alpha, test_list_sigma, cols, alphas_col)
    p_all = plot(ps_test... , layout = (4, 1))
    savefig(string("plots/", bifur_type, "_entropy_single_data_summ_stat_plot_rerun_more_sdes.pdf"))
