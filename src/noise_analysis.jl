function solve_prob_alpha_sigma_new(prob_arg_list, p_arg) # More efficient version
    temp_f, temp_g, temp_tspan, initpoints = prob_arg_list
    temp_p = p_arg
    Npoints = 20 # keeping this low is good for performance
    timepoints = 0.2:0.2:2.0 # changing stepsize doenst cost much but makes results smoother. changed start from 0.2 ( why was it 0.2 before?).
    temp_outputs = []
